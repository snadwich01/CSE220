...
public class AssignmentTask2{
    ...
    public static Node organizeBooks(Node head, Integer[] popularity) {
        ...
        while(current.next != null) {
            ...
            while(next != null) {
                if(popularity[nex] > popularity[maxpos]) {
                    max = next;
                    maxpos = nex;
                }
                next = next.next;
            ...
            if(max != current) {
                String tempbook = (String) current.elem;
                current.elem = max.elem;
                max.elem = tempbook;

                int tempos = popularity[curr];
                popularity[curr] = popularity[maxpos];
                popularity[maxpos] = tempos;
            }
