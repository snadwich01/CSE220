public class StackLabTaskTester {
    ...
    public static void removeBlock(Stack stack, int n) {
        // Temporary stack to hold elements while we access the nth block
    Stack temp = new Stack();

    // Pop the top (n-1) elements and push into temp
    for (int i = 1; i < n && !stack.isEmpty(); i++) {
        temp.push(stack.pop());
    }

    // Remove the nth element (simply pop it)
    if (!stack.isEmpty()) {
        stack.pop();
    }
